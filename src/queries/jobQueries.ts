...
/* --------------------------------------------
   Vendor scan helpers
   - startCratingJob: set Status = 'Crating', set StartTime if null
   - completeCratingJob: set Status = 'CratingComplete', set EndTime if null
---------------------------------------------*/
export async function startCratingJob(
  jobId: number,
  areaId?: number | null
): Promise<CratingJob | null> {
  const pool = await connectDB()
...
export async function completeCratingJob(
  jobId: number
): Promise<CratingJob | null> {
  const pool = await connectDB()
  const request = pool.request()
  ...
  const result = await request.query<CratingJob>(`
      UPDATE core.CratingJob
      SET
        Status  = 'CratingComplete',
        EndTime = COALESCE(EndTime, SYSUTCDATETIME())
      OUTPUT
        INSERTED.Job_Id       AS jobId,
        INSERTED.SerialNumber AS serialNumber,
        INSERTED.Model        AS model,
